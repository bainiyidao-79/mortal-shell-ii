...
echo "内容页产物："
COUNT=$(find out \( -path "*/guides/*" -o -path "*/shells/*" -o -path "*/weapons/*" -o -path "*/beacons/*" -o -path "*/collectibles/*" -o -path "*/walkthrough/*" \) -name "*.html" | wc -l)
find out \( -path "*/guide/*" -o -path "*/info/*" \) -name "*.html" | sort
if [ "$COUNT" -eq 0 ]; then
